# CJDNS support in Xero Core

It is possible to run Xero Core over CJDNS, an encrypted IPv6 network that
...
done outside of Xero Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Xero network and individual nodes.
...
before it will work with your Xero Core node. This is described in steps
...
## Run Xero Core with CJDNS

Once you are connected to the CJDNS network, the following Xero Core
...
When enabled, this option tells Xero Core that it is running in an
...
IPv6 local network. This helps Xero Core perform better address management:
...
CJDNS support was added to Xero Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `xero-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/xero/xero/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Xero Core

There are several ways to see your CJDNS address in Xero Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `xero-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `xero-cli getpeerinfo`).
