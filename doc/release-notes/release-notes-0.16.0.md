Xero Core version 0.16.0 is now available from:

  <https://bitcoincore.org/bin/xero-core-0.16.0/>
...
Xero Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
xero data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Xero Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µXRO" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- xero-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- xero-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 35169.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest xero.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.xero.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for xero-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to xero-cli to allow RPC password to be read from standard input (jharvell)
...
- #10825 `28485c7` Set regtest JSON-RPC port to 18443 to avoid conflict with testnet 35169 (fametrano)
...
- #12035 `eeb6d52` Change µXRO to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_xero_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` xero_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.xero doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for xero-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in xero-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop xerod (OmeGak)
...
- #10529 `331352f` Improve xerod systemd service file (Flowdalic)
