...
""" Example logging Xero Core mempool events using the mempool:added,
...
def main(xerod_path):
    xerod_with_usdts = USDT(path=str(xerod_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    xerod_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    xerod_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    xerod_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    xerod_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[xerod_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/xerod")
        exit(1)
