...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Xero address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new XeroAddressEntryValidator(parent));
    widget->setCheckValidator(new XeroAddressCheckValidator(parent));
}
...
bool parseXeroURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no xero: URI
    if(!uri.isValid() || uri.scheme() != QString("xero"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!XeroUnits::parse(XeroUnit::XRO, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseXeroURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseXeroURI(uriInstance, out);
}

QString formatXeroURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("xero:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(XeroUnits::format(XeroUnit::XRO, info.amount, false, XeroUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openXeroConf()
{
    ...
    /* Open xero.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Xero.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Xero (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Xero (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "xero.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("xero-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a xero.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Xero\n";
        else
            optionFile << strprintf("Name=Xero (%s)\n", chain);
