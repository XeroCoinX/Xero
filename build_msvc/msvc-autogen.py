...
def parse_config_into_xro_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Xero Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/xero_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/xero_config.h'), "w", encoding="utf8") as xro_config:
        xro_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Xero-core msbuild configuration initialiser.')
    ...
    parse_config_into_xro_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/xero_config.h'), os.path.join(SOURCE_DIR, 'config/xero-config.h'))
