...
/** Call node.updateRwSetting() with Xero 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Xero 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/xero/xero/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because xero 22.x will not try to read these.
...
//! Convert enabled/size values to xero -prune setting.
...
//! Get pruning enabled value to show in GUI from xero -prune setting.
...
//! Get pruning size value to show in GUI from xero -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayXeroUnit")) {
        settings.setValue("DisplayXeroUnit", QVariant::fromValue(XeroUnit::XRO));
    }
    QVariant unit = settings.value("DisplayXeroUnit");
    if (unit.canConvert<XeroUnit>()) {
        m_display_xero_unit = unit.value<XeroUnit>();
    } else {
        m_display_xero_unit = XeroUnit::XRO;
        settings.setValue("DisplayXeroUnit", QVariant::fromValue(m_display_xero_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if xero.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_xero_unit);
...
{
    if (new_unit.isNull() || new_unit.value<XeroUnit>() == m_display_xero_unit) return;
    m_display_xero_unit = new_unit.value<XeroUnit>();
    QSettings settings;
    settings.setValue("DisplayXeroUnit", QVariant::fromValue(m_display_xero_unit));
    Q_EMIT displayUnitChanged(m_display_xero_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/xero/xero/pull/8273
    ...
    // (https://github.com/xero-core/gui/issues/567).
