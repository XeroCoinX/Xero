...
class XeroTestMetaClass(type):
    """Metaclass for XeroTestFramework.

    Ensures that any attempt to register a subclass of `XeroTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'XeroTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("XeroTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("XeroTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class XeroTestFramework(metaclass=XeroTestMetaClass):
    """Base class for a xero test script.

    Individual xero test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave xerods and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop xerods after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use xero-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_xerod = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "xerod" + config["environment"]["EXEEXT"],
        )
        fname_xerocli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "xero-cli" + config["environment"]["EXEEXT"],
        )
        fname_xeroutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "xero-util" + config["environment"]["EXEEXT"],
        )
        self.options.xerod = os.getenv("XEROD", default=fname_xerod)
        self.options.xerocli = os.getenv("XEROCLI", default=fname_xerocli)
        self.options.xeroutil = os.getenv("XEROUTIL", default=fname_xeroutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: xerods were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("XeroRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'xerod', self.options.xerod) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'xero-cli', self.options.xerocli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a xerod test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple xerod test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as xerod's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    xerod=self.options.xerod,
                    xero_cli=self.options.xerocli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in xero.conf
    ...
    def skip_if_no_xerod_tracepoints(self):
        """Skip the running test if xerod has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("xerod has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_xerod_zmq(self):
        """Skip the running test if xerod has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("xerod has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if xero-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("xero-wallet has not been compiled")

    def skip_if_no_xero_util(self):
        """Skip the running test if xero-util has not been compiled."""
        if not self.is_xero_util_compiled():
            raise SkipTest("xero-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if xero-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("xero-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether xero-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether xero-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_xero_util_compiled(self):
        """Checks whether xero-util was compiled."""
        return self.config["components"].getboolean("ENABLE_XERO_UTIL")
