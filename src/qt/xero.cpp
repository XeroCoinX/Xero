...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<XeroUnit>("XeroUnit");
#else
    qRegisterMetaType<XeroUnit>("XeroUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. xero_de.qm (shortcut "de" needs to be defined in xero.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. xero_de_DE.qm (shortcut "de_DE" needs to be defined in xero.qrc)
...
static const char* qt_argv = "xero-qt";

XeroApplication::XeroApplication()
...
void XeroApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the XeroApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", XeroGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void XeroApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool XeroApplication::createOptionsModel(bool resetSettings)
{
...
void XeroApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new XeroGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &XeroGUI::quitRequested, this, &XeroApplication::requestShutdown);
...
void XeroApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void XeroApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool XeroApplication::baseInitialize()
{
    return node().baseInitialize();
}

void XeroApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &XeroApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &XeroApplication::handleRunawayException);
    connect(this, &XeroApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &XeroApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void XeroApplication::parameterSetup()
{
...
void XeroApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void XeroApplication::requestInitialize()
{
...
void XeroApplication::requestShutdown()
{
...
void XeroApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // xero: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &XeroGUI::handlePaymentRequest);
            connect(window, &XeroGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void XeroApplication::handleRunawayException(const QString &message)
{
...
void XeroApplication::handleNonFatalException(const QString& message)
{
...
WId XeroApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool XeroApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", XeroGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(xero);
    Q_INIT_RESOURCE(xero_locale);
    ...
    /// 6-7. Parse xero.conf, determine network, switch to network specific
    ...
    // xero: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
