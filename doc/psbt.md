# PSBT Howto for Xero Core

Since Xero Core 0.17, an RPC interface exists for Partially Signed Xero
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/xero/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Xero transactions that are not fully signed
...
#### Multisig with multiple Xero Core instances
...
Xero Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `xero-cli` is used.
...
Later, when *V* XRO has been received on *Amulti*, and Bob and Carol want to
