...
const std::string XeroGUI::DEFAULT_UIPLATFORM =
...
XeroGUI::XeroGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &XeroGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &XeroGUI::showModalOverlay);
...
void XeroGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Xero address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and xero: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &XeroGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &XeroGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &XeroGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Xero addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Xero addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Xero Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Xero Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a xero: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Xero command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &XeroGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &XeroGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &XeroGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &XeroGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &XeroGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &XeroGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &XeroGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &XeroGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &XeroGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &XeroGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &XeroGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &XeroGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &XeroGUI::showDebugWindow);
}

void XeroGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void XeroGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &XeroGUI::setCurrentWalletBySelectorIndex);
...
void XeroGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &XeroGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &XeroGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &XeroGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &XeroGUI::showProgress);
...
#ifdef ENABLE_WALLET
void XeroGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void XeroGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &XeroGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &XeroGUI::removeWallet);
...
WalletController* XeroGUI::getWalletController()
{
    return m_wallet_controller;
}

void XeroGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &XeroGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &XeroGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &XeroGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &XeroGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &XeroGUI::incomingTransaction);
    connect(this, &XeroGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void XeroGUI::removeWallet(WalletModel* walletModel)
{
...
void XeroGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void XeroGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void XeroGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void XeroGUI::setWalletActionsEnabled(bool enabled)
{
...
void XeroGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &XeroGUI::toggleHidden);
...
void XeroGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void XeroGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void XeroGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void XeroGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void XeroGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void XeroGUI::openClicked()
{
...
void XeroGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void XeroGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void XeroGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void XeroGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void XeroGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void XeroGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void XeroGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void XeroGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Xero network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void XeroGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void XeroGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void XeroGUI::updateHeadersSyncProgressLabel()
{
...
void XeroGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void XeroGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &XeroGUI::quitRequested);
...
void XeroGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void XeroGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void XeroGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &XeroGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &XeroGUI::show);
                e->ignore();
            }
        }
    }
...
void XeroGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void XeroGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void XeroGUI::incomingTransaction(const QString& date, XeroUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(XeroUnits::formatWithUnit(unit, amount, true));
...
void XeroGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void XeroGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool XeroGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool XeroGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void XeroGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void XeroGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void XeroGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void XeroGUI::updateWindowTitle()
{
...
void XeroGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void XeroGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void XeroGUI::detectShutdown()
{
...
void XeroGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void XeroGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(XeroGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = XeroGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void XeroGUI::subscribeToCoreSignals()
{
...
void XeroGUI::unsubscribeFromCoreSignals()
{
...
bool XeroGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<XeroUnit> units = XeroUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const XeroUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, XeroUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const XeroUnit u : XeroUnits::availableUnits()) {
        menu->addAction(XeroUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(XeroUnit newUnits)
{
    setText(XeroUnits::longName(newUnits));
}
