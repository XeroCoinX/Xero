...
const char * const XERO_CONF_FILENAME = "xero.conf";
const char * const XERO_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //xero-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", XERO_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Xero
    // macOS: ~/Library/Application Support/Xero
    // Unix-like: ~/.xero
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Xero";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Xero";
#else
    // Unix-like
    return pathRet / ".xero";
#endif
...
{
    return GetConfigFile(*this, GetPathArg("-conf", XERO_CONF_FILENAME));
}
