# User-space, Statically Defined Tracing (USDT) for Xero Core

Xero Core includes statically defined tracepoints to allow for more
...
**Note**: Xero Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Xero Core
...
Multiple tools can list the available tracepoints in a `xerod` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Xero Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/xerod
stap net        outbound_message 0x0000000000107c05 /src/xerod
stap validation block_connected  0x00000000002fb10c /src/xerod
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Xero Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/xerod | grep NT_STAPSDT -A 4 -B 2
