...
Below are some notes on how to build Xero Core for Windows.

The options known to work for building Xero Core on Windows are:
...
Note that for WSL the Xero Core source path MUST be somewhere in the default mount file system, for
example /usr/src/xero, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\xero`, for example:

    make install DESTDIR=/mnt/c/workspace/xero
