When Xero Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Xero node is listening,
or one where a service is listening that is not related to the Xero network.
As a result, this service may occasionally get connection attempts from Xero
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Xero Core,
trying to connect because it thinks there is a Xero node on that
...
Below is a list of "bad" ports which Xero Core avoids when choosing a peer to
...
[pull/23306](https://github.com/xero/xero/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/xero/xero/pull/23542)
