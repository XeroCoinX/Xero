...
The Xero Core daemon can be configured to act as a trusted "border
router", implementing the xero wire protocol and relay, making
...
The ZeroMQ feature in Xero Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building xerod:
...
For instance:
    ...
    $ xerod -zmqpubhashtx=tcp://127.0.0.1:225169 \
               -zmqpubhashtx=tcp://192.168.1.2:225169 \
               -zmqpubhashblock="tcp://[::1]:225170" \
               -zmqpubrawtx=ipc:///tmp/xerod.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in xero.conf.
...
From the perspective of xerod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into xerod directly. Furthermore, no information is
...
using. Xerod appends an up-counting sequence number to each
