...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing xero-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='xero-cli', help='xero-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Xero address to which the faucet should send')
...
parser.add_argument('xero_cli_args', nargs='*', help='Arguments to pass on to xero-cli (default: -signet)')

args = parser.parse_args()

if args.xero_cli_args == []:
    args.xero_cli_args = ['-signet']


def xero_cli(rpc_command_and_params):
    argv = [args.cmd] + args.xero_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = xero_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = xero_cli(['getnewaddress', 'faucet', 'bech32'])
